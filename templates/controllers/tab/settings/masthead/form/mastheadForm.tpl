...
<form class="pkp_form" id="mastheadForm" method="post" action="{url router=$smarty.const.ROUTE_COMPONENT component="tab.settings.JournalSettingsTabHandler" op="saveFormData" tab="masthead"}">
	...
	{fbvFormArea id="mastheadFacultaTematica"}
		{fbvFormSection label="manager.setup.facultad" for="facultad" description="manager.setup.facultadDescription"}
			{fbvElement type="text" multilingual=true name="facultad" id="facultad" value=$facultad maxlength="255"}
		{/fbvFormSection}
		
		{fbvFormSection label="manager.setup.tematica" for="tematica" description="manager.setup.tematicaDescription"}
			{fbvElement type="text" multilingual=true name="tematica" id="tematica" value=$tematica maxlength="255"}
		{/fbvFormSection}
